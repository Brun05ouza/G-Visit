...
import { CalendarCheck, Clock, MapPin, User, Plus, X, Mail, MessageSquare, Phone, Trash2 } from "lucide-react";
import { getVisitsByDate, deleteVisit } from "@/lib/firestore";
...
export default function Home() {
  ...
  const [selectedVisit, setSelectedVisit] = useState<VisitWithId | null>(null);
      ...
      <AnimatePresence>
        {selectedVisit && (
          ...
          >
            ...
            <motion.div
              className="relative w-full max-w-md rounded-2xl border border-[#283842] bg-[#202C33] p-5 shadow-xl sm:p-6"
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
            ...
            >
              <div className="mb-4 flex items-center justify-between border-b border-[#283842] pb-4">
                <h3 className="text-lg font-bold text-white">Detalhes da visita</h3>
                <button
                  type="button"
                  onClick={() => setSelectedVisit(null)}
                  className="flex h-8 w-8 items-center justify-center rounded-lg text-[#8A9BA8] transition-colors hover:bg-[#2A3C46] hover:text-white"
                  aria-label="Fechar"
                >
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-3">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#10BCA9]/10">
                    <User className="h-4 w-4 text-[#10BCA9]" />
                  </div>
                  <div>
                    <p className="text-xs font-medium text-[#8A9BA8]">Nome</p>
                    <p className="text-[15px] font-semibold text-white">{selectedVisit.name}</p>
                  </div>
                </div>

                <div className="flex items-start gap-3">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#183A40]">
                    <Phone className="h-4 w-4 text-[#10BCA9]" />
                  </div>
                  <div>
                    <p className="text-xs font-medium text-[#8A9BA8]">Telefone</p>
                    <p className="text-[15px] text-white">{selectedVisit.phone}</p>
                  </div>
                </div>

                <div className="flex items-start gap-3">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#183A40]">
                    <Mail className="h-4 w-4 text-[#10BCA9]" />
                  </div>
                  <div>
                    <p className="text-xs font-medium text-[#8A9BA8]">E-mail</p>
                    <p className="text-[15px] text-white">{selectedVisit.email}</p>
                  </div>
                </div>

                <div className="flex items-start gap-3">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#183A40]">
                    <Clock className="h-4 w-4 text-[#10BCA9]" />
                  </div>
                  <div>
                    <p className="text-xs font-medium text-[#8A9BA8]">Data e horário</p>
                    <p className="text-[15px] text-white">
                      {formatDisplayDate(selectedVisit.preferredDate)} às {selectedVisit.preferredTime}
                      <span className="ml-2 text-[13px] text-[#8A9BA8]">
                        ({selectedVisit.timeFlexibility === "flexible" ? "Flexível" : "Fixo"})
                      </span>
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-xs font-medium text-[#8A9BA8]">Imóvel de interesse</p>
                    <p className="text-[15px] text-white">{selectedVisit.propertyInterest}</p>
                  </div>
                </div>
                ...
                {selectedVisit.message && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-xs font-medium text-[#8A9BA8]">Mensagem</p>
                      <p className="text-[15px] text-white">{selectedVisit.message}</p>
                    </div>
                  </div>
                )}
              </div>

              <div className="mt-6 flex flex-wrap items-center gap-2 border-t border-[#283842] pt-4">
                <motion.button
                  type="button"
                  onClick={() => window.open(`https://wa.me/55${selectedVisit.phone.replace(/\D/g, "")}`, "_blank")}
                  className="flex shrink-0 items-center justify-center gap-1.5 rounded-lg bg-[#10BCA9] px-3 py-2 text-[13px] font-semibold text-white transition-colors hover:bg-[#0EA998] sm:rounded-xl sm:px-4 sm:py-2.5"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  <svg className="h-4 w-4 shrink-0 sm:h-[18px] sm:w-[18px]" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                  </svg>
                  WhatsApp
                </motion.button>
                <motion.button
                  type="button"
                  disabled={cancelingId === selectedVisit.id}
                  onClick={async () => {
                    if (!selectedVisit) return
                    setCancelingId(selectedVisit.id)
                    try {
                      ...
                      toast.error("Erro ao cancelar visita. Tente novamente.")
                    } finally {
                      setCancelingId(null)
                    }
                  }}
                  className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-red-500/50 bg-red-500/10 px-3 py-2 text-[13px] font-semibold text-red-400 transition-colors hover:bg-red-500/20 disabled:opacity-50 sm:flex-initial sm:rounded-xl sm:px-4 sm:py-2.5"
                  whileHover={{ scale: cancelingId === selectedVisit.id ? 1 : 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  <Trash2 className="h-4 w-4 shrink-0 sm:h-[18px] sm:w-[18px]" strokeWidth={2.5} />
                  {cancelingId === selectedVisit.id ? "Cancelando…" : "Cancelar visita"}
                </motion.button>
                <motion.button
                  type="button"
                  onClick={() => setSelectedVisit(null)}
                  className="shrink-0 rounded-lg border border-[#405461] bg-[#2E3D46] px-4 py-2 text-[13px] font-semibold text-[#E6EEF3] transition-colors hover:bg-[#384A54] sm:rounded-xl sm:px-5 sm:py-2.5 sm:text-[15px]"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  Fechar
